...
function normalizeBindings(path, replacement, targetIdentifier) {
  return {
    ObjectProperty(p) {
      if (!(p.scope !== path.scope && p.scope.hasOwnBinding(targetIdentifier.name)) && p.node.shorthand && isIdentifier(p.node.key) && p.node.key.name === targetIdentifier.name && isIdentifier(p.node.value) && p.node.value.name === targetIdentifier.name) {
        p.replaceWith(objectProperty(targetIdentifier, replacement));
      }
    },
    Expression(p) {
      if (isIdentifier(p.node) && p.node.name === targetIdentifier.name && !(p.scope !== path.scope && p.scope.hasOwnBinding(targetIdentifier.name))) {
  ...
  if (isObjectPattern(pattern)) {
    for (let i = 0, len = pattern.properties.length; i < len; i += 1) {
      ...
      if (isObjectProperty3(property)) {
        ...
        } else if (isAssignmentPattern(value)) {
        } else {
    ...
    for (let i = 0, len = pattern.elements.length; i < len; i += 1) {
      ...
      if (property) {
        ...
        } else if (isArrayPattern(property) || isObjectPattern(property)) {
...
function createCallbackLabel(label, named = false) {
  return function expr(state, path) {
    ...
    let name;
    let callback;
    if (named && isLabeledStatement(body)) {
...
var EXPRESSIONS = {
  ...
  destructure: createVariableLabel((state, path, leftExpr, rightExpr) => {
    if ((isObjectPattern2(leftExpr) || isArrayPattern2(leftExpr)) && rightExpr) {
  ...
  path.traverse({
    VariableDeclarator(p) {
      if (p.parentPath === path) {
        const leftExpr = p.node.id;
        const rightExpr = p.node.init;
        if ((isObjectPattern3(leftExpr) || isArrayPattern3(leftExpr)) && rightExpr) {
...
function rootExpression(state, path) {
  ...
  const arrow = isArrowFunctionExpression(argument) || isFunctionExpression(argument) ? argument : arrowFunctionExpression6([], argument);
...
function destructureExpression2(state, path) {
  ...
  const leftExpr = path.parent.id;
  if (!(isObjectPattern4(leftExpr) || isArrayPattern4(leftExpr))) {
...
var CTF_PARSER = {
  CallExpression(path, state) {
    if (isIdentifier7(path.node.callee) && path.node.callee.name in CTF_EXPRESSIONS && !path.scope.hasBinding(path.node.callee.name)) {
...
var AUTO_IMPORT = {
  Expression(p, state) {
    if (isIdentifier8(p.node) && !p.scope.hasBinding(p.node.name) && p.node.name in AUTO_IMPORT_EXPR) {
