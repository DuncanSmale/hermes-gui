...
import { batch, createContext, useContext, mergeProps, createEffect, onCleanup, createMemo, createSignal, createComputed, createResource, untrack } from "solid-js";
...
const useQueryClient = () => {
  const queryClient = useContext(getQueryClientContext(useContext(QueryClientSharingContext)));
  if (!queryClient) {
    throw new Error("No QueryClient set, use QueryClientProvider to set one");
  }
  return queryClient;
};
const QueryClientProvider = (_props) => {
  _props = mergeProps({}, {
  ...
  createEffect(() => {
    _props.client.mount();
    onCleanup(() => {
      _props.client.unmount();
    });
  });
  const Context = getQueryClientContext(_props.contextSharing);
  return createComponent(QueryClientSharingContext.Provider, {
    get value() {
      return _props.contextSharing;
    },
    get children() {
      return createComponent(Context.Provider, {
        get value() {
          return _props.client;
        },
        get children() {
...
const QueryErrorResetBoundary = (_props) => {
  const value = createMemo(() => createValue());
  return createComponent(QueryErrorResetBoundaryContext.Provider, {
    get value() {
      return value();
    },
    get children() {
      return memo(() => typeof _props.children === "function", true)() ? _props.children(value) : _props.children;
    }
  });
};
function noop() {
...
function createMutation(arg1, arg2, arg3) {
  ...
  return __spreadProps(__spreadValues({}, currentResult), {
    mutate,
...
function createBaseQuery(options, Observer) {
  ...
  createComputed(() => {
    options.queryKey = typeof options.queryKeySignal === "function" ? options.queryKeySignal() : options.queryKeySignal;
  ...
  const observer = createMemo(() => new Observer(queryClient, dOptions()));
  const getResult = () => observer().getOptimisticResult(dOptions());
  const [data, {
    refetch,
    mutate
  }] = createResource(() => dOptions().queryKey, () => {
...
function createQuery(queryKey, queryFn, options) {
  return createBaseQuery(__spreadValues({
...
function useQueries(queries) {
  const queryClient = useQueryClient();
  const defaultedQueries = createMemo(() => queries.map((options) => {
...
const Hydrate = (_props) => {
  useHydrate(_props.state, _props.options);
  return memo(() => _props.children);
};
export { CancelledError, Hydrate, InfiniteQueryObserver, MutationCache, MutationObserver, QueriesObserver, QueryCache, QueryClient, QueryClientProvider, QueryErrorResetBoundary, QueryObserver, createMutation, createQuery, dehydrate, focusManager, hashQueryKey, hydrate, isCancelledError, isError, notifyManager, onlineManager, setLogger, useHydrate, useInfiniteQuery, useIsFetching, useIsMutating, useQueries, useQueryClient, useQueryErrorResetBoundary };
