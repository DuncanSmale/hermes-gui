...
import require$$1 from '@babel/types';
...
var helperModuleImports = lib$1;

var htmlEntities = lib;
...
const ChildProperties = new Set(["innerHTML", "textContent", "innerText", "children"]); // React Compat
...
"set", "stop", // "style",
"svg", "switch", "symbol", "text", "textPath", // "title",
...
function filterChildren(children) {
  return children.filter(({
    node: child
  }) => !(t__namespace.isJSXExpressionContainer(child) && t__namespace.isJSXEmptyExpression(child.expression)) && (!t__namespace.isJSXText(child) || !/^[\r\n]\s*$/.test(child.extra.raw)));
}
...
function transformCondition(path, inline, deep) {
  ...
  if (t__namespace.isConditionalExpression(expr) && (isDynamic(path.get("consequent"), {
    ...
    if (dTest) {
      ...
      id = inline ? t__namespace.callExpression(memoWrapper, [t__namespace.arrowFunctionExpression([], cond), t__namespace.booleanLiteral(true)]) : path.scope.generateUidIdentifier("_c$");
    ...
    let nextPath = path; // handle top-level or, ie cond && <A/> || <B/>
    ...
    if (dTest) {
      ...
      if (!t__namespace.isBinaryExpression(cond)) cond = t__namespace.unaryExpression("!", t__namespace.unaryExpression("!", cond, true), true);
      id = inline ? t__namespace.callExpression(t__namespace.identifier(`_$${config.memoWrapper}`), [t__namespace.arrowFunctionExpression([], cond), t__namespace.booleanLiteral(true)]) : path.scope.generateUidIdentifier("_c$");
      nextPath.node.left = t__namespace.callExpression(id, []);
    }
  }

  if (dTest && !inline) {
    const statements = [t__namespace.variableDeclaration("const", [t__namespace.variableDeclarator(id, config.memoWrapper ? t__namespace.callExpression(t__namespace.identifier(`_$${config.memoWrapper}`), [t__namespace.arrowFunctionExpression([], cond), t__namespace.booleanLiteral(true)]) : t__namespace.arrowFunctionExpression([], cond))]), t__namespace.arrowFunctionExpression([], expr)];
...
function setAttr$2(path, elem, name, value, {
  ...
  if (name === "style") {
    ...
    return t__namespace.callExpression(style, prevId ? [elem, value, prevId] : [elem, value]);
  }
  ...
  if (name === "classList") {
    ...
    return t__namespace.callExpression(classList, prevId ? [elem, value, prevId] : [elem, value]);
  }
  ...
  if (ns) {
    ...
    return t__namespace.callExpression(setAttributeNS, [elem, t__namespace.stringLiteral(ns), t__namespace.stringLiteral(name), value]);
  } else {
...
function transformAttributes$2(path, results) {
  ...
  const tagName = getTagName(path.node),
        ...
        config = getConfig(path); // preprocess styles
  ...
  if (classAttributes.length > 1) {
    const first = classAttributes[0].node,
          values = [],
          quasis = [t__namespace.TemplateElement({
      raw: ""
    })];
    ...
    if (values.length) first.value = t__namespace.JSXExpressionContainer(t__namespace.TemplateLiteral(quasis, values));else first.value = t__namespace.stringLiteral(quasis[0].value.raw);
  }
  ...
  path.get("openingElement").get("attributes").forEach(attribute => {
    ...
    if (t__namespace.isJSXSpreadAttribute(node)) {
      let spread = registerImportMethod(attribute, "spread", getRendererConfig(path, "dom").moduleName);
      results.exprs.push(t__namespace.expressionStatement(t__namespace.callExpression(spread, [elem, isDynamic(attribute.get("argument"), {
        checkMember: true
      }) ? t__namespace.isCallExpression(node.argument) && !node.argument.arguments.length ? node.argument.callee : t__namespace.arrowFunctionExpression([], node.argument) : node.argument, t__namespace.booleanLiteral(isSVG), t__namespace.booleanLiteral(hasChildren)]))); //NOTE: can't be checked at compile time so add to compiled output
    ...
    let value = node.value,
        key = t__namespace.isJSXNamespacedName(node.name) ? `${node.name.namespace.name}:${node.name.name.name}` : node.name.name,
        reservedNameSpace = t__namespace.isJSXNamespacedName(node.name) && reservedNameSpaces.has(node.name.namespace.name);
    ...
    if (t__namespace.isJSXExpressionContainer(value) && (reservedNameSpace || !(t__namespace.isStringLiteral(value.expression) || t__namespace.isNumericLiteral(value.expression)))) {
      if (key === "ref") {
        ...
        results.exprs.unshift(t__namespace.expressionStatement(t__namespace.callExpression(t__namespace.identifier(node.name.name.name), [elem, t__namespace.arrowFunctionExpression([], t__namespace.isJSXEmptyExpression(value.expression) ? t__namespace.booleanLiteral(true) : value.expression)])));
        ...
        if (key.startsWith("on:") || key.startsWith("oncapture:")) {
          ...
          if (t__namespace.isArrayExpression(handler)) {
            ...
            let addEventListener = registerImportMethod(path, "addEventListener", getRendererConfig(path, "dom").moduleName);
            results.exprs.unshift(t__namespace.expressionStatement(t__namespace.callExpression(addEventListener, [elem, t__namespace.stringLiteral(ev), handler, t__namespace.booleanLiteral(true)])));
          }
        } else {
          let handler = value.expression;
          const resolveable = detectResolvableEventHandler(attribute, handler);
          ...
          if (t__namespace.isArrayExpression(handler)) {
            ...
            results.exprs.unshift(t__namespace.expressionStatement(t__namespace.callExpression(t__namespace.memberExpression(elem, t__namespace.identifier("addEventListener")), [t__namespace.stringLiteral(ev), handler])));
          } else if (t__namespace.isFunction(handler) || resolveable) {
            results.exprs.unshift(t__namespace.expressionStatement(t__namespace.callExpression(t__namespace.memberExpression(elem, t__namespace.identifier("addEventListener")), [t__namespace.stringLiteral(ev), handler])));
          } else {
            let addEventListener = registerImportMethod(path, "addEventListener", getRendererConfig(path, "dom").moduleName);
            results.exprs.unshift(t__namespace.expressionStatement(t__namespace.callExpression(addEventListener, [elem, t__namespace.stringLiteral(ev), handler])));
          }
        }
        ...
        results.exprs.push(t__namespace.expressionStatement(setAttr$2(attribute, elem, key, value.expression, {
          isSVG,
          isCE
        })));
      }
      ...
      if (value && ChildProperties.has(key)) {
        results.exprs.push(t__namespace.expressionStatement(setAttr$2(attribute, elem, key, value, {
          isSVG,
          isCE
        })));
      } else {
...
function transformChildren$2(path, results, config) {
  ...
  const filteredChildren = filterChildren(path.get("children")),
        childNodes = filteredChildren.map((child, index) => transformNode(child, {
  ...
  childNodes.forEach((child, index) => {
    ...
    if (child.id) {
      ...
      results.decl.push(t__namespace.variableDeclarator(child.id, config.hydratable && tagName === "html" ? t__namespace.callExpression(getNextMatch, [walk, t__namespace.stringLiteral(child.tagName)]) : walk));
      ...
      if (markers || wrappedByText(childNodes, index)) {
        ...
        results.exprs.push(t__namespace.expressionStatement(t__namespace.callExpression(insert, [results.id, child.exprs[0], nextChild$1(childNodes, index) || t__namespace.nullLiteral()])));
      } else {
        results.exprs.push(t__namespace.expressionStatement(t__namespace.callExpression(insert, config.hydratable ? [results.id, child.exprs[0], t__namespace.identifier("undefined"), t__namespace.callExpression(t__namespace.memberExpression(t__namespace.memberExpression(t__namespace.memberExpression(t__namespace.identifier("Array"), t__namespace.identifier("prototype")), t__namespace.identifier("slice")), t__namespace.identifier("call")), [t__namespace.memberExpression(results.id, t__namespace.identifier("childNodes")), t__namespace.numericLiteral(0)])] : [results.id, child.exprs[0]])));
      }
...
function createPlaceholder(path, results, tempPath, i, char) {
  ...
  if (config.hydratable && char === "/") {
    let getNextMarker = registerImportMethod(path, "getNextMarker", getRendererConfig(path, "dom").moduleName);
    contentId = path.scope.generateUidIdentifier("co$");
    results.decl.push(t__namespace.variableDeclarator(t__namespace.arrayPattern([exprId, contentId]), t__namespace.callExpression(getNextMarker, [t__namespace.memberExpression(t__namespace.identifier(tempPath), t__namespace.identifier("nextSibling"))])));
...
function detectExpressions(children, index, config) {
  if (children[index - 1]) {
    ...
    if (t__namespace.isJSXExpressionContainer(node) && !t__namespace.isJSXEmptyExpression(node.expression) && !getStaticExpression(children[index - 1])) return true;
    let tagName;
  ...
  for (let i = index; i < children.length; i++) {
    ...
    if (t__namespace.isJSXExpressionContainer(child)) {
      ...
      if (child.openingElement.attributes.some(attr => t__namespace.isJSXSpreadAttribute(attr) || ["textContent", "innerHTML", "innerText"].includes(attr.name.name) || attr.name.namespace && attr.name.namespace.name === "use" || t__namespace.isJSXExpressionContainer(attr.value) && !(t__namespace.isStringLiteral(attr.value.expression) || t__namespace.isNumericLiteral(attr.value.expression)))) return true;
...
function contextToCustomElement(path, results) {
  ...
  results.exprs.push(t__namespace.expressionStatement(t__namespace.assignmentExpression("=", t__namespace.memberExpression(results.id, t__namespace.identifier("_$owner")), t__namespace.callExpression(getOwner, []))));
}
...
function createTemplate$2(path, result, wrap) {
  ...
  if (result.id) {
    ...
    if (!(result.exprs.length || result.dynamics.length || result.postExprs.length) && result.decl.declarations.length === 1) {
      ...
      return t__namespace.callExpression(t__namespace.arrowFunctionExpression([], t__namespace.blockStatement([result.decl, ...result.exprs.concat(wrapDynamics$1(path, result.dynamics) || [], result.postExprs || []), t__namespace.returnStatement(result.id)])), []);
    }
  }
...
function appendTemplates$1(path, templates) {
  const declarators = templates.map(template => {
    ...
    return t__namespace.variableDeclarator(template.id, t__namespace.callExpression(templateImport, [t__namespace.templateLiteral([t__namespace.templateElement(tmpl, true)], []), t__namespace.numericLiteral(template.elementCount)].concat(template.isSVG ? t__namespace.booleanLiteral(template.isSVG) : [])));
  });
...
function registerTemplate(path, results) {
  ...
  if (results.template.length) {
    ...
    let getNextElement;
    hydratable && (getNextElement = registerImportMethod(path, "getNextElement", getRendererConfig(path, "dom").moduleName));
    decl = t__namespace.variableDeclarator(results.id, hydratable ? t__namespace.callExpression(getNextElement, templateId ? [templateId] : []) : t__namespace.callExpression(t__namespace.memberExpression(templateId, t__namespace.identifier("cloneNode")), [t__namespace.booleanLiteral(true)]));
  }
...
function wrapDynamics$1(path, dynamics) {
  ...
  if (dynamics.length === 1) {
    ...
    return t__namespace.expressionStatement(t__namespace.callExpression(effectWrapper, [t__namespace.arrowFunctionExpression(prevValue ? [prevValue] : [], setAttr$2(path, dynamics[0].elem, dynamics[0].key, dynamics[0].value, {
  ...
  return t__namespace.expressionStatement(t__namespace.callExpression(effectWrapper, [t__namespace.arrowFunctionExpression([prevId], t__namespace.blockStatement([t__namespace.variableDeclaration("const", decls), ...statements, t__namespace.returnStatement(prevId)])), t__namespace.objectExpression(identifiers.map(id => t__namespace.objectProperty(id, t__namespace.identifier("undefined"))))]));
}
...
function escapeExpression(path, expression, attr) {
  ...
  return t__namespace.callExpression(escape, [expression].concat(attr ? [t__namespace.booleanLiteral(true)] : []));
}
...
function transformToObject(attrName, attributes, selectedAttributes) {
  ...
  for (let i = 0; i < selectedAttributes.length; i++) {
    ...
    properties.push(t__namespace.objectProperty(computed ? t__namespace.stringLiteral(attr.name.name.name) : t__namespace.identifier(attr.name.name.name), t__namespace.isJSXExpressionContainer(attr.value) ? attr.value.expression : attr.value));
...
function transformAttributes$1(path, results) {
  ...
  const tagName = getTagName(path.node),
        ...
        hasChildren = path.node.children.length > 0,
        attributes = path.get("openingElement").get("attributes"),
        styleAttributes = attributes.filter(a => t__namespace.isJSXNamespacedName(a.node.name) && a.node.name.namespace.name === "style"),
  ...
  const classAttributes = attributes.filter(a => a.node.name && (a.node.name.name === "class" || a.node.name.name === "className" || a.node.name.name === "classList")); // combine class propertoes
  ...
  attributes.forEach(attribute => {
    ...
    if (t__namespace.isJSXSpreadAttribute(node)) {
      ...
      appendToTemplate(results.template, " ");
      results.template.push("");
      results.templateValues.push(t__namespace.callExpression(ssrSpread, [isDynamic(attribute.get("argument"), {
        checkMember: true,
        native: true
      }) ? t__namespace.isCallExpression(node.argument) && !node.argument.arguments.length ? node.argument.callee : t__namespace.arrowFunctionExpression([], node.argument) : node.argument, t__namespace.booleanLiteral(isSVG), t__namespace.booleanLiteral(hasChildren)]));
      return;
    }
    ...
    if (t__namespace.isJSXExpressionContainer(value) && (reservedNameSpace || ChildProperties.has(key) || !(t__namespace.isStringLiteral(value.expression) || t__namespace.isNumericLiteral(value.expression)))) {
      if (key === "ref" || key.startsWith("use:") || key.startsWith("prop:") || key.startsWith("on")) return;else if (ChildProperties.has(key)) {
        children = value;
        ...
        if (BooleanAttributes.has(key)) {
          ...
          results.template.push("");
          const fn = t__namespace.callExpression(ssrBoolean, [t__namespace.stringLiteral(key), value.expression]);
        ...
        if (key === "classList") {
          if (t__namespace.isObjectExpression(value.expression) && !value.expression.properties.some(p => t__namespace.isSpreadElement(p))) {
            ...
            if (!values.length) value.expression = t__namespace.stringLiteral(quasis[0].value.raw);else if (values.length === 1 && !quasis[0].value.raw && !quasis[1].value.raw) {
        ...
        if (dynamic) value.expression = t__namespace.arrowFunctionExpression([], doEscape ? escapeExpression(path, value.expression, true) : value.expression);else if (doEscape) value.expression = escapeExpression(path, value.expression, true);
...
function transformClasslistObject(path, expr, values, quasis) {
  expr.properties.forEach((prop, i) => {
    ...
    let key = prop.key;
    if (t__namespace.isIdentifier(prop.key) && !prop.computed) key = t__namespace.stringLiteral(key.name);else if (prop.computed) {
      let escape = registerImportMethod(path, "escape");
...
function transformChildren$1(path, results, {
  ...
  filteredChildren.forEach(node => {
    ...
    if (child.exprs.length) {
      ...
      if (!doNotEscape) child.exprs[0] = escapeExpression(path, child.exprs[0]); // boxed by textNodes
...
function createTemplate$1(path, result) {
  ...
  return t__namespace.callExpression(ssr, result.template.length > 1 ? [id, ...result.templateValues] : [id]);
}
...
function transformElement$1(path, info) {
  ...
  results.decl.unshift(t__namespace.variableDeclarator(results.id, t__namespace.callExpression(createElement, results.args ? [t__namespace.stringLiteral(tagName), results.args] : [t__namespace.stringLiteral(tagName)])));
  return results;
}
...
function transformAttributes(path, results) {
  ...
  path.get("openingElement").get("attributes").forEach(attribute => {
    ...
    if (t__namespace.isJSXSpreadAttribute(node)) {
      ...
      results.exprs.push(t__namespace.expressionStatement(t__namespace.callExpression(spread, [elem, isDynamic(attribute.get("argument"), {
        checkMember: true
      }) ? t__namespace.isCallExpression(node.argument) && !node.argument.arguments.length ? node.argument.callee : t__namespace.arrowFunctionExpression([], node.argument) : node.argument, t__namespace.booleanLiteral(hasChildren)])));
      return;
    }
...
function setAttr(path, elem, name, value, {
  ...
  return t__namespace.callExpression(setProp, prevId ? [elem, t__namespace.stringLiteral(name), value, prevId] : [elem, t__namespace.stringLiteral(name), value]);
}
...
function transformChildren(path, results) {
  ...
  childNodes.forEach((child, index) => {
    ...
    if (child.id) {
      ...
      if (child.text) {
        if (multi) {
          results.decl.push(t__namespace.variableDeclarator(child.id, t__namespace.callExpression(registerImportMethod(path, "createTextNode", getRendererConfig(path, "universal").moduleName), [t__namespace.stringLiteral(htmlEntities.decode(child.template))])));
        } else insert = t__namespace.callExpression(registerImportMethod(path, "createTextNode", getRendererConfig(path, "universal").moduleName), [t__namespace.stringLiteral(htmlEntities.decode(child.template))]);
      }
      ...
      if (multi) {
        results.exprs.push(t__namespace.expressionStatement(t__namespace.callExpression(insert, [results.id, child.exprs[0], nextChild(childNodes, index) || t__namespace.nullLiteral()])));
      } else {
...
function createTemplate(path, result, wrap) {
  ...
  if (result.id) {
    ...
    if (!(result.exprs.length || result.dynamics.length || result.postExprs.length) && result.decl.declarations.length === 1) {
      ...
      return result.dynamicArgs ? t__namespace.callExpression(t__namespace.callExpression(registerImportMethod(path, config.memoWrapper), [t__namespace.arrowFunctionExpression([], template)]), []) : template;
    } else {
      let template = t__namespace.arrowFunctionExpression([], t__namespace.blockStatement([result.decl, ...result.exprs.concat(wrapDynamics(path, result.dynamics) || [], result.postExprs || []), t__namespace.returnStatement(result.id)]));
      return t__namespace.callExpression(result.dynamicArgs ? t__namespace.callExpression(registerImportMethod(path, config.memoWrapper), [template]) : template, []);
    }
  }
...
function wrapDynamics(path, dynamics) {
  ...
  if (dynamics.length === 1) {
    ...
    return t__namespace.expressionStatement(t__namespace.callExpression(effectWrapper, [t__namespace.arrowFunctionExpression([prevValue], setAttr(path, dynamics[0].elem, dynamics[0].key, dynamics[0].value, {
      dynamic: true,
      prevId: prevValue
    }))]));
  }
...
function transformComponent(path) {
  ...
  path.get("openingElement").get("attributes").forEach(attribute => {
    ...
    if (t__namespace.isJSXSpreadAttribute(node)) {
      ...
      props.push(isDynamic(attribute.get("argument"), {
        checkMember: true
      }) && (dynamicSpread = true) ? t__namespace.isCallExpression(node.argument) && !node.argument.arguments.length ? node.argument.callee : t__namespace.arrowFunctionExpression([], node.argument) : node.argument);
    } else {
      ...
      if (t__namespace.isJSXExpressionContainer(value)) {
        if (key === "ref") {
          if (config.generate === "ssr") return; // Normalize expressions for non-null and type-as
          ...
          let expr = config.wrapConditionals && config.generate !== "ssr" && (t__namespace.isLogicalExpression(value.expression) || t__namespace.isConditionalExpression(value.expression)) ? transformCondition(attribute.get("value").get("expression"), true) : t__namespace.arrowFunctionExpression([], value.expression);
          runningObject.push(t__namespace.objectMethod("get", wrapName(key), [], t__namespace.blockStatement([t__namespace.returnStatement(expr.body)]), !t__namespace.isValidIdentifier(key)));
  ...
  if (childResult && childResult[0]) {
    if (childResult[1]) {
      const body = t__namespace.isCallExpression(childResult[0]) ? t__namespace.isFunction(childResult[0].callee) ? childResult[0].callee.body : childResult[0] : childResult[0].body;
  ...
  exprs.push(t__namespace.callExpression(createComponent, componentArgs)); // handle hoisting conditionals
...
function transformThis(path) {
  ...
  return node => {
    if (thisId) {
      ...
      const decl = t__namespace.variableDeclaration("const", [t__namespace.variableDeclarator(thisId, t__namespace.thisExpression())]);
...
function transformNode(path, info = {}) {
  ...
  if (t__namespace.isJSXElement(node)) {
    ...
    return {
      exprs: expr.length > 1 ? [t__namespace.callExpression(t__namespace.arrowFunctionExpression([], t__namespace.blockStatement([expr[0], t__namespace.returnStatement(expr[1])])), [])] : [expr],
      template: "",
      dynamic: true
    };
...
function transformElement(config, path, info = {}) {
  const node = path.node;
  let tagName = getTagName(node); // <Component ...></Component>

  if (isComponent(tagName)) return transformComponent(path); // <div ...></div>
...
var postprocess = path => {
  if (path.scope.data.events) {
    ...
    path.node.body.push(t__namespace.expressionStatement(t__namespace.callExpression(delegateEvents, [t__namespace.arrayExpression(Array.from(path.scope.data.events).map(e => t__namespace.stringLiteral(e)))])));
  }
...
var jsxTransform = babelPluginJsxDomExpressions;

var solid = ((context, options = {}) => {
  const plugins = [[jsxTransform, Object.assign({
...
function solidPlugin(options = {}) {
  ...
  return {
    ...
    async transform(source, id, transformOptions) {
      ...
      const ssr = transformOptions === true || transformOptions?.ssr;
